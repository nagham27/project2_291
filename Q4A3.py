...
def userOptimized():
    dbCursor.execute('PRAGMA automatic_index = true;')
    dbCursor.execute('CREATE INDEX customer_id_index ON Orders(customer_id);')

def solveQuery():
    
    script = '''
    SELECT COUNT (DISTINCT Sellers.seller_postal_code)
    FROM Customers , Orders , Sellers 
    WHERE Customers.customer_id = Orders.customer_id AND Customers.customer_id = (SELECT customer_id
    FROM Orders
...
def dropIndices():
    dbCursor.execute('DROP INDEX customer_id_index')
...
def drawGraph(uninformedPlot, selfPlot, userPlot):
    ...
    axes.set_title('Q4 (Runtime in ms)')
    axes.legend()
