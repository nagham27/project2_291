...
def solveQuery(randomCode):
    ...
    JOIN (
    SELECT order_id, COUNT(order_item_id) AS size
    FROM Order_items
    GROUP BY order_id
    ) os ON oi.order_id = os.order_id
    WHERE c.customer_postal_code = {randomCode}
    AND oi.order_id IN (
    SELECT oi2.order_id
    ...
    FROM Order_items
    GROUP BY order_id
    ) os2 ON oi2.order_id = os2.order_id
    WHERE os2.size > os.size
...
def drawGraph(uninformedPlot, selfPlot, userPlot):
    ...
    axes.set_title('Q3 (Runtime in ms)')
    axes.legend()
